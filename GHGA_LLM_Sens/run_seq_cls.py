...
from paddle.metric import Accuracy, Recall, Precision

from gpu_util import show_gpu
...
def main():
    ...
    raw_datasets['train'] = MapDataset(train)
    test = pickle.load(open('test.pkl', 'rb'))
    raw_datasets['test']= MapDataset(test)
    dev = pickle.load(open('dev.pkl', 'rb'))
    raw_datasets['dev'] = MapDataset(dev)
    # print(dev)
    data_args.label_list = ['0', '1', '2', '3']
    num_classes = len(data_args.label_list)
    # raw_datasets=load_dataset('clue',data_args.dataset)
    # data_args.label_list = getattr(raw_datasets["train"], "label_list", None)
    # num_classes = len(data_args.label_list)
    # print(data_args.label_list)
    ...
    if training_args.do_predict:
        ...
        out_file = open(os.path.join(training_args.output_dir, "ERNIE3-Sens_predict.json"), "w")
